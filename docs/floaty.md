...
## floaty.isCircularMenuShowing()
* 返回 {boolean}

查询 AutoJS 自带的悬浮小球（悬浮菜单）当前是否显示。

## floaty.hideCircularMenu()
* 返回 {boolean}

隐藏 AutoJS 自带的悬浮小球（悬浮菜单），返回值为**隐藏前**该小球是否处于显示状态。

脚本运行时通常靠截图识别、点击屏幕来工作，自带的悬浮小球会遮挡屏幕、干扰截图识别或造成误触。可在脚本开始时调用本方法隐藏小球，并用返回值记录其原状态，结束后再据此还原。

隐藏操作会同步切换到主线程执行（等待执行完成后才返回），因此隐藏后可紧接着进行截图。

## floaty.showCircularMenu()
* 返回 {boolean}

显示（还原）AutoJS 自带的悬浮小球（悬浮菜单）。若小球已在显示则不会重复创建。返回值表示该接口在当前环境是否可用。

典型的"运行时隐藏、结束后还原"用法：

```js
var wasShowing = floaty.hideCircularMenu();
try {
    // ... 脚本主体（截图、识别、点击等）...
} finally {
    if (wasShowing) floaty.showCircularMenu();
}
```
